...
import {
  ...
  applyMovement,
  calculateMovement,
  calculateIdeologyInfluenceChange,
...
export const gameMachine = setup({
  ...
  actions: {
    ...
    resolveTurn: assign({
      ...
      players: ({ context }) => {
        ...
        for (const [playerId, player] of newPlayers) {
          ...
          // Calculate influence change based on ideology alignment (only if vote passed)
          let newInfluence = player.influence;
          if (voteResult.passed && option) {
            const influenceChange = calculateIdeologyInfluenceChange(player.ideology, option);
            newInfluence = applyInfluenceChange(player.influence, influenceChange);
          }

          newPlayers.set(playerId, { ...player, position: newPosition, influence: newInfluence });
        }
        return newPlayers;
      },
      history: ({ context }) => {
    ...
    advanceToNextPlayer: assign({
      ...
      pendingVotes: () => new Map<string, Vote>(),
      timerEndAt: () => null,
    }),
